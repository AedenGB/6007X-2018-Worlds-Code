...
float scaleTo(float *values, float scale = 127){//take two numbers, and make them both under 127 while presering relative size and sign
     ...
     values[1] = 127/fabs(values[0])*values[1];//scale second value
     values[0] = 127*sgn(values[0]);//limit first value
   }
   else {//second value determining factorn(larger)
     values[0] = 127/fabs(values[1])*values[0];//scale first value
     values[1] = 127*sgn(values[1]);//limit second vale
   }
   return values;
}
